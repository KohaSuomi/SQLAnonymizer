...
subtest "SQLAnon::Filters::dateOfBirthAnonDayMonth", sub {
  my $dob;
  eval {
    ok($dob = SQLAnon::Filters->dateOfBirthAnonDayMonth('borrowers', 'dateofbirth', {dateofbirth => '1968-03-22'}),
...
subtest "SQLAnon::Filters::kohaSystempreferences", sub {
  ...
  eval {
    ...
    ok($val = SQLAnon::Filters->kohaSystempreferences('systempreference', 'value', {variable => 'VaaraAcqVendorConfigurations', value => 'anonymizable value'}),
       'Given filter params, we receive a filtered syspref');
    is($val, "---\n\n",
       'syspref filtered');

    ok($val = SQLAnon::Filters->kohaSystempreferences('systempreference', 'value', {variable => 'AutoSelfCheckPass', value => 'anonymizable value'}),
       'Given filter params, we receive a filtered syspref');
    is($val, "1234",
       'syspref filtered');
  };
...
subtest "SQLAnon::Filters::addressWithSuffix", sub {
  ...
  eval {
    $oldVal = 'Haimakatu 55';
    ok($val = SQLAnon::Filters->addressWithSuffix('borrowers', 'address', {address => $oldVal}),
...
subtest "SQLAnon::Filters->killIfTimestampOlderThanYear", sub {
  ...
  eval {
    ...
    is(SQLAnon::Filters->killIfTimestampOlderThanYear('borrowers', 'address', {address => $oldVal}), $oldVal,
       'timestamp not older than year');

    $oldVal = '2015-12-05';
    is(SQLAnon::Filters->killIfTimestampOlderThanYear('borrowers', 'address', {address => $oldVal}), $kill,
       'timestamp kills');
...
subtest "SQLAnon::Filters->randomString", sub {
  ...
  eval {
    $oldVal = '167A001234';
    ok($val = SQLAnon::Filters->randomString('borrowers', 'cardnumber', {cardnumber => $oldVal}),
       'Given a cardnumber, we receive a anonymized string');
    is(length($val), $SQLAnon::Filters::stringLength,
       'With correct length');

    ok($val2 = SQLAnon::Filters->randomString('borrowers', 'cardnumber', {cardnumber => $oldVal}),
    ...
    for (0..10000) {
      SQLAnon::Filters->randomString('borrowers', 'cardnumber', {cardnumber => rand(9999)}, 0);
    }
